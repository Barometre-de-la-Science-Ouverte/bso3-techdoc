...
Recently, pro-active and voluntarist policies to enforce higher standards of openness and visibility for all research results have been introduced. National Open Science policies in particular are currently rapidly developing. One outstanding example is the French nationa plan for Open Science, ...

To evaluate, adapt and maximize the adoption of these policies, their effect must be measured. Monitoring tool and dashboards are crucial to follow the evoluation of practice regarding openness and sharing of dataset and software. However, in contrast with the well established practice of citing publication, the visibility of research datasets and software is considered largely insufficient and challenging. 
...
However, when they exist, the PID associated to software are currently not used [@du_peerj_2022]. Similarly, [@10.1002/asi.24049] indicate that most data use indicators are informal (mentions without any identifiers) and found mostly in footnotes, acknowledgement or supplementary materials sections of research publication. Still regarding PID usage for dataset citations, He and Han [@he_han_2017] report that less than 10% of publications that include data mentions contain any PID. According to the study of [@macgregor2022exploring], PID associated to data have a much more limited awareness among researchers as compared to publication PID. 
...
In view of the current limited adoption and impact of PID for dataset and software until now, one immediately available alternative to PID for dataset and software is to rely on a text mining approach to detect automatically mentions of these research results in full text and their role in research publications. If the full text corpus of scientific publications is comprehensive enough, this approach can provide a reliable snapshot of the actual openness and sharing practices of research datasets and software at a given time, and lead to the creation of indicators monitoring the impact of open science policies.

For extending the French Open Science Monitor (BSO) [platform](https://frenchopensciencemonitor.esr.gouv.fr) to measure the openness of dataset and software, we choosed this approach and implemented a large text mining approach applied to a vast corpus of scientific publications including at least one author with a French affiliation. We will describe in this paper that recent advances in scientific text mining make indeed possible fine-grained bottom-up approaches to capture reliably the dataset and software mentions directly from the full text publications, as well as the characterization of these mentions in term of usage, creation and sharing. 
...
### Limitation of existing tools: ODDPub

In the context of Open Science and bibliometrics, some pattern matching tools have been developed to capture statements about data and software openness in a given corpus. For example, [@Larregue_2020] uses some keyword matching applied in data availability statements to obtain an approximation of the data availability status for 7,394 COVID-19 articles deposited on medRxiv. We know of course that data sharing statements are not limited to data availability statements, COVID-19 articles are not representative of general data sharing practices and keyword matching is not a state of the art automation techniques at the time of Deep Learning. Such works can be seen more as one shot exercices for reporting interesting results than a reliable and re-usable method for mining data openness and sharing in full texts.  

As a more general purpose tool, ODDPub [@10.1002/pra2.614] has been developed to detect open data statements in full texts and applied at scale to 2.75 million articles on PubMed Central publications [@10.1002/pra2.614]. It implements a rule-based approach for capturing open data statement patterns. ODDPub gives then global information about openess of data and software used in a document. The rules have been developed for biomedical literature. However, we think ODDPub, like the other similar pattern matching tools, has fundamental limitations for the production of data and code Open Science indicators: 

- ODDPub produce information about **openness** of data and software used in the work described in a publication, but does not produce information about their re-use, creation and sharing. Therefore, it not possible to estimate the volume of novel data and code. In particular, it makes impossible to estimate the key ratio of created data/software openly shared, that Open Science policies want to monitor and maximize. Similarly it is not possible to estimate the ratio of re-use of open data or software as compared to all data and code used in set of publications. 
...
- Reported F1-score are 0.73 for data openness recognition and 0.64 for code openness recognition at document level. As indicated in [@10.1002/pra2.614], performance metrics for the open code detection are unreliable because of a lack of data for significant evaluation (open code in the evaluation data was found in 11 publications). Overall, the reported accuracy is lower than what is usually expected for reliable text mining at scale []. 

- ODDPub has often no clear distinction between code and data - including in the evaluation data of this tool. Data present on a GitHub repository for example, will be often classified as open software (see below).

- ODDPub produces global screening information about openness at document level, but the tool is not usable to identify reliably mentions and to provide usable information at software and dataset levels. 

Below, an example of an ODDPub dataset manually checked entry used for the development of the tool. This mismatch between data and code openness statements is actually quite frequent in the validation dataset of ODDPub, up to 40% for code according to our evaluation. 
